...
import jease.site.i18n.Strings;
...
public class Discussions {
	...
	public static String addComment(Discussion discussion, String author,
		...
		if (Validations.isEmpty(subject) || Validations.isEmpty(author)
				|| Validations.isEmpty(comment)) {
			return Strings.All_fields_are_required;
		}
		if (subject.length() > MAX_SUBJECT_LENGTH
				|| author.length() > MAX_AUTHOR_LENGTH
				|| comment.length() > MAX_COMMENT_LENGTH) {
			return Strings.Input_is_too_long;
		}

		// Clean up submitted parameters before processing.
		subject = Regexps.stripTags(subject);
		author = Regexps.stripTags(author);
		comment = Regexps.stripTags(comment);
		...
		if (Validations.isNotEmpty(recipient)) {
